...
const PRICING_CATALOG = [
  ...
  },
  {
    ...
    aliases: new Set(["gpt-5.1", "gpt-5.1-codex-max", "gpt-5.1-codex-mini"]),
  },
  {
    modelId: "gpt-5-codex",
    inputUsdPerMillion: 1.25,
...
export function estimateUsageWrappedCost(tokenTotalsByModel) {
  ...
  addUnique(notes, "Reasoning tokens are estimated at the model's output-token rate.");
...
function priceUsd(pricing, totals) {
  const billedOutputTokens = (totals.output || 0) + (totals.reasoning || 0);
  return (
    ((totals.input || 0) / TOKENS_PER_MILLION) * pricing.inputUsdPerMillion +
